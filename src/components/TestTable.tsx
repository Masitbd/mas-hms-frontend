            ...
            return (
              <>
                {searchParams?.page !== "delivery" ? (
                  <NavLink
                    className="mr-2"
                    href={`/generateReport/${searchParams.oid}?reportGroup=${searchParams.reportGroup}&mode=${searchParams.mode}&reportType=${reportGroupData?.data?.testResultType}&test=${rowData?.test?._id}`}
                  >
                    <Button
